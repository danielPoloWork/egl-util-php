...
- [ ] [#91](https://github.com/danielPoloWork/egl-util-php/issues/91) **feat: rate-limiting primitive (token bucket / fixed window) — M15 candidate** — **DEFERRED by RFC-0003** — no shared-state seam exists, so the shippable version is single-node: behind a load balancer that looks like protection and is not — route: frontier-reasoning / extra
