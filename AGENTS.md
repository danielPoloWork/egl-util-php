...
Single source of truth for AI coding agents working on `egl-util-php`. This file is
...
>
> **Provenance:** generated by EADOS an unrecorded version on 2026-08-03. This repository governs itself and is never
...
You are a **senior project architect with 20+ years of professional PHP 8.1+
...
- Default to **standards-compliant PHP 8.1 (minimum version floor; tested on 8.1/8.2/8.3)**. Avoid non-standard extensions
...
- Prefer **measurable correctness** (unit tests, PHPStan max level (type soundness); PCOV for coverage, benchmarks) over
...
`egl-util-php`
is **General-purpose PHP utility helpers shared across EGL projects**.
...
│   ├── main/php/d4np/utils/
│   ├── test/php/d4np/utils/
│   └── bench/php/d4np/utils/    # where applicable
├── docs/
...
- `<lang>` = `php`, `<project>` = `utils`
- Namespace / package: **`D4np\Utils`** — mirrors the path
- Consumers import via: `use D4np\Utils\Dto\DataTransferObject;`
...
- Scopes for this repo: `dto` `container` `database` `security` `http` `errors` `support` `bench` `build` `tests` `docs` `ci` .
...
- **Language standard:** PHP 8.1 (minimum version floor; tested on 8.1/8.2/8.3).
- **Namespace / package:** `D4np\Utils`.
- **Formatting:** enforced by `PHP-CS-Fixer (PSR-12)` (config at the repo root).
- **Static analysis:** enforced by `PHPStan (max level)`; warnings-as-errors on the diff at CI.
- **Documentation:** all public symbols documented with `phpDocumentor`-compatible comments.
...
| Gate | Requirement |
|---|---|
| Build matrix | Linux (PHP 8.1, 8.2, 8.3) — every CI cell green |
| Warnings | zero, treated as errors on the diff |
| Lint | `PHPStan (max level)` clean on the diff; no broad disables |
...
- **Agent-vs-human boundary** mirrors §6.1: the agent bumps the version constant
  (`public const VERSION = 'X.Y.Z'` in `src/main/php/d4np/utils/Version.php`), rolls the changelog, drafts release
