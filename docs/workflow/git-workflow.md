...
Full conventions for branches, commits, and pull requests on `egl-util-php`. The short
...
- `scope` is a short subsystem noun. Scopes for this repo: `dto` `container` `database` `security` `http` `errors` `support` `bench` `build` `tests` `docs` `ci` .
