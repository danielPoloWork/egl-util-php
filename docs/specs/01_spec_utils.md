...
- NFR-15 Sortable identifier generation (FR-46): `Str::ulid()` and `Str::uuidV7()` each <= 10 us, CI ceiling (item 14.2, ADR-0063). Budgeted because an identifier is drawn once per inserted row, so its cost rides every write — the one path in FR-46's surface paid at volume rather than once at wiring. Derived, not chosen: measured on the reference runner at **3.453 us** (ULID) and **2.592 us** (UUIDv7), and ADR-0058's rule places a never-firing ceiling at >= 2.66x the worst reading (3.453 x 2.66 = 9.19), so 10 us is the next round number above that band — 2.90x and 3.86x respectively. **A refuted assumption is recorded with it**: `Str::random(10)` was expected to be the floor beneath both, and measured **6.083 us** — slower than either, because it draws `random_int()` once per character while these draw `random_bytes()` once. The scopes are not nested, so NFR-15 rests on the identifiers' own readings alone (item 10.10's lesson, this time catching the error before the budget was written rather than three items later)
