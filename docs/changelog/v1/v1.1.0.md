...
The first release after the freeze. Milestone 14 adds five additive seams to the frozen 1.0
surface — a PSR-20 clock, sortable identifiers, pagination values, detached HMAC signatures and
a rate limiter — and Milestone 13 closes the documentation and release-hygiene debt the v1.0.0
review board logged. **Nothing in the 1.0 API changed**: every entry below is additive under
ADR-0059, so `^1.0` continues to resolve.
...
- **Five new seams, all additive.** `Support\SystemClock`/`FrozenClock` (PSR-20),
  `Str::ulid()`/`Str::uuidV7()`, `Persistence\PageRequest`/`Page`, `Security\Hmac`, and
  `Security\RateLimiter` with its store abstraction. Nothing existing changed shape.
- **The dist is production code only.** `export-ignore` cuts the Packagist archive from 524
  files to 120 — no tests, benchmarks, ADRs, tooling or CI config in your `vendor/`.
- **A published API reference**, built and gate-checked on every pull request.
- **The README has a consumer on-ramp** — install, a naming map, and four examples that were
  each executed against the published package rather than proof-read.
- **Three CI gates that could not previously fail now can**: per-diff coverage, documentation
  link resolution, and the API-docs build.
...
- **Milestone 14, the first functional roadmap after the freeze** (issue #84), specified by the
  newly accepted [RFC-0003](../../rfc/0003-post-1-0-functional-scope.md). Five numbered items, all
...
- **Supported-versions window for the post-1.0 line**, defined in
  [`docs/workflow/maintenance.md`](../../workflow/maintenance.md) and pointed to from `SECURITY.md`:
...
- **The constant-time comparison registry had been blind to every call in the library**
  ([BUG-0001](../../bugs/2026/08/BUG-0001-constant-time-registry-blind-to-prefixed-calls.md), the
