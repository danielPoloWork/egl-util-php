...
What is in the box, for a reader who does not need the engineering record below to use this
release. Full detail — every decision, probe, and measurement behind each line — is in the
narrative log that follows; nothing here says anything the log does not already say at length.

- **The public API is frozen.** `composer require egl/utils:^1.0` is now a safe constraint: no
  signature is removed or altered within the 1.x line, and a deprecation ships one full published
  MINOR before removal (**ADR-0059**).
- **Eight component groups**, each usable on its own: typed immutable **DTOs** with strict
  hydration; a minimal PSR-11 **container**; safe-by-default PDO access with a fluent query
  builder (**Database**); **security** helpers — context-aware escaping, sanitization, Argon2id
  hashing, AES-256-GCM encryption; an **HTTP** layer that refuses rather than coerces — hardened
  sessions/CSRF, an outbound client, a router, one JSON envelope shape; PSR-3 **error** channels
  with level filtering and fan-out; validated **mail** that cannot carry a header injection; and
  the **support** utilities (`Str`, `File`, `Csv`, `Url`, `Lookup`, …) the rest build on.
- **Nothing here guesses.** A typed accessor returns its default instead of coercing bad input to
  a string; a header carrying `CR`/`LF` is refused when it is set, not when it is sent; an unknown
  log level throws even when the floor would have dropped the record. Refusing loudly, rather than
  degrading silently, is the design stance across every group.
- **Verified, not asserted**: 2 831 tests on PHP 8.1, 8.2 and 8.3; ≥ 90% line coverage; PHPStan at
  max level; 79% mutation score against a 70% floor; ten absolute performance budgets plus a
  same-runner regression gate.
- **Known limits, stated rather than discovered** — see [`docs/releases/v1.0.0.md`](../../releases/v1.0.0.md)
  for the full list (a Windows-only mailer quirk, `EmailAddress`'s stricter-than-RFC-5321 stance,
  the PSR-7 bridge's publication status) and **§ "How this release was published"** for the tag's
  signing status, which this file's own *Superseded pre-release* note below also covers.
...
- **`D4np\Utils\Http\ApiEnvelope` + `Http\Outcome`** — one JSON shape for every answer an API
  ...
  **`caught()` takes a correlation reference, not a `Throwable`** — an envelope built from an
  ...
  [`docs/patterns/endpoint-kernel.md`](../../patterns/endpoint-kernel.md).
...
- **Internal function calls are prefixed with `\` throughout the library** (roadmap item **10.12**;
  **ADR-0048**; [benchmark record](../../benchmarks/2026/08/native-function-invocation-repo-wide.md)),
...
- **`RowNormalizer` is faster on the default policy** (roadmap item **10.11**; **ADR-0047**;
  [benchmark record](../../benchmarks/2026/08/nfr09-rownormalizer-trim-only-fast-path.md)). The
...
- **The PSR-7 bridge packaging decision** (**ADR-0033**, closing Milestone 7 and RFC-0001's A-8) —
  plus [`docs/specs/02_spec_psr7_bridge.md`](../../specs/02_spec_psr7_bridge.md), the frozen contract
...
- `D4np\Utils\Dto\HydrationCompiler` (**ADR-0013**) closes NFR-01's performance gap: hydrating a
  ...
  [`docs/benchmarks/2026/08/nfr01-hydration-compiled-closure.md`](../../benchmarks/2026/08/nfr01-hydration-compiled-closure.md).
