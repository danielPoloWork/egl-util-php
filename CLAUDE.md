...
- **Persona:** senior project architect with 20+ years of enterprise PHP 8.1+
...
- **Source layout:** Maven-style cross-language tree. All code under
  `src/main/php/d4np/utils/` (tests under `src/test/...`,
  benchmarks under `src/bench/...`). Namespace `D4np\Utils`. See §5.
...
- **Quality bar:** enterprise — warnings-as-errors, `PHPStan (max level)` clean, `PHP-CS-Fixer (PSR-12)`
  clean, PHPStan max level (type soundness); PCOV for coverage green, `phpDocumentor` documented, `consistency_lint.py` passing. No
  shortcuts. See §10.
