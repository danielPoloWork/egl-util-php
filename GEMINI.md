...
- **Source layout:** Maven-style cross-language tree. All code under
  `src/main/php/d4np/utils/`. Namespace `D4np\Utils`. See §5.
...
- **Quality bar:** enterprise — warnings-as-errors, `PHPStan (max level)` clean, `PHP-CS-Fixer (PSR-12)`
  clean, PHPStan max level (type soundness); PCOV for coverage green, `consistency_lint.py` passing. No shortcuts. See §10.
